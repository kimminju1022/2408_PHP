...
/** 사용코드 및 고민코드별 정의
 * !in_array : 논리 연산자로, 배열에 특정 값이 존재하지 않는지 확인하는 함수. 
 *           - 반대되는 값으로 in_array 함수가 있으며, 배열에서 특정 값이 존재하는지 검사 후 존재하면 true, 아니라면 false를 반환
 ...
 *  5인 이유 : 
        * 배열의 요소 개수가 5보다 작은지 확인하는 조건으로 이 조건이 참일 때만 코드 블록이 실행됨
        ...
        1. 배열이 빈 상태에서 시작
        2. 배열에 요소를 추가할 때마다 count($lotto_lucky)의 값이 책정됨
...
while (count($lotto_lucky) < 5) {
    $randombox = array_rand($lotto_num);
    $random_num = $lotto_num[$randombox];
    if (!in_array($random_num, $lotto_lucky)) {
        $lotto_lucky[] = $random_num;
    }
}
